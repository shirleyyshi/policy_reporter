...
- backend healthcheck 排障（2026-08-24 首次上线踩坑）：healthcheck 失败会让 frontend 因 `service_healthy` 依赖起不来；应急恢复用 `docker compose up -d --no-deps frontend`。**真实根因是 ALLOWED_HOSTS**：探活绕过 Nginx 直连 127.0.0.1:8000，`Host: 127.0.0.1` 不在 `.env` 的 ALLOWED_HOSTS 里，`DEBUG=False` 下 Django 返回 400（不是代理/网络问题）。修复：settings.py 固定向 ALLOWED_HOSTS 追加回环地址（127.0.0.1/localhost/[::1]），与业务域名配置解耦；回归测试用 `HTTP_HOST='127.0.0.1'` 复现原故障。
